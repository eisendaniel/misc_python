...
class Drawer:
    ...
    def draw(self, x0, y0, x1, y1, mode):
        if mode == "line":
            return self.canvas.create_line(x0, y0, x1, y1, fill=self.color, tags=(str(len(self.actions))))

        elif mode == "pen":
            r = self.canvas.create_line(x0, y0, x1, y1, tags=(str(len(self.actions))))
            self.startx = x1
            self.starty = y1
            return r

        elif mode == "rect":
            return self.canvas.create_rectangle(x0, y0, x1, y1, fill=self.color, width=0, tags=(str(len(self.actions))))

        elif mode == "oval":
            return self.canvas.create_oval(x0, y0, x1, y1, fill=self.color, width=0, tags=(str(len(self.actions))))

    def undo(self, event):
        if len(self.actions) == 0:
            return
        self.actions.pop()
        self.canvas.delete((str(len(self.actions))))
        self.canvas.delete(self.tmp)
